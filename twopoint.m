function [trial_matrix, index_sequence, offset_matrix, numcones, exp_sequence, stimpattern, stimseparation, selectionimage] = twopoint(CFG)

[I stimposx stimposy] = deal([]); % Needed to instantiate nested variable access
[stimwidth,stimheight] = deal(CFG.selectionWidthtwopoint); % Stimulus total width and height
...
if exist('lastPattern.mat','file')==2  % ask for redo
    choice = questdlg('Earlier stimulus pattern detected.', ...
        'Redo?', ...
...
stimseparation = D;
selectionimage = SI;
save('lastPattern.mat', 'stimpattern','stimseparation','selectionimage');
...
numtrials = numcones.*CFG.delta;
ncatch = 0; nlapse = 0;
Intensity_Vector = CFG.thresholdGuess;
...
exp_sequence(:,2) = CFG.thresholdGuess; % All patterns are presented with the same intensity.
...
% CORRECTION of light intensity for single dot case (zero pixel distance), currently intensity
% set to twice as much as two dot case
% TODO: make this more reasonable
if (CFG.thresholdGuess*2 <= 1)
    exp_sequence(exp_sequence(:,1)==1,2) = CFG.thresholdGuess*2; % double intensity
else
    exp_sequence(exp_sequence(:,1)==1,2) = 1; % cap condition 
end
...
exp_sequence(numtrials+1:end,:)=[];  % Workaround to clip the sequence after numtrials trials. 
...
% Show ROI and let user select cones from zoomed image
    function selectPattern (hObject,eventdata,h)
        ...
        titletext = {'Leftclick selects cones. First is anchor.','Press any other key when ready'};
        h.title = title(titletext,'FontSize',14);
        ...
        while but == 1
            ...
            plot(x,y,'g+');
            text(x+2,y,num2str(conecount),'Color','g');
            xi(conecount,1) = x;
            yi(conecount,1) = y;
            conecount = conecount+1;
        end
        f = getframe(gcf);
        selectionimage = f.cdata;
        SI = selectionimage;
        x0 = xi(1,1); y0 = yi(1,1);
        close
        ...
        for j =1:N
            Y(j,j) = 1;
            D(j,1) = sqrt((xi(j,1)-x0)^2+(yi(j,1)-y0)^2);
        end
       
        % Determine pixel distance from anchor (=first location)
        
        
        
        
        % Z = [sum(Y,2) Y]; Z = sortrows(Z); Y = Z(:,2:end); Y = fliplr(Y); % sort to preserve click order
        % Y = fliplr(Y); % sort to preserve click order
        npatterns = size(Y,1);
        
        
        % Make stimuli based off of selected pixels
        for j = 1: npatterns % start at 2, otherwise the all-zero combination would be tested also
            cone_x = round(xi(Y(j,:)==1)); cone_y = round(yi(Y(j,:)==1));
            stimpatterns(:,:,j) = makeStimulus(int8(stimwidth),int8(stimheight),dotsize,dottype,cone_x,cone_y,antialias);
        end
        ...
        h.text = uicontrol('Style','Text','Position',[42 5 190 39],...
            'String',['Patterns to test: ',num2str(npatterns)],'FontWeight','Bold',...
