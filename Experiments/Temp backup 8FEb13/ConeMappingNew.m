        ...
        elseif strcmp(resp,kb_StimConst)    % check if present stimulus button was pressed
            if PresentStimulus == 1;
                ...
                if CFG.red_stim_color == 1;
                    ...
                    aom1offx(1,:) = x_off; %TCA correction
                    Mov.aom1offx = aom1offx;
                    aom1offy = zeros(1,size(aom1seq,2));
                    aom1offy(1,:) = y_off;  %TCA correction
                    Mov.aom1offy = aom1offy;
                elseif CFG.red_stim_color == 0;
                    aom1offx = zeros(1,size(aom1seq,2));
                    Mov.aom1offx = aom1offx;
                ...
                if CFG.green_stim_color == 1;
                    ...
                    aom2offx(1,:) = x_off; %TCA correction
                    Mov.aom2offx = aom2offx;
                    aom2offy = zeros(1,size(aom1seq,2));
                    aom2offy(1,:) = y_off; %TCA correction
                    Mov.aom2offy = aom2offy;
                elseif CFG.green_stim_color == 0;
                    aom2offx = zeros(1,size(aom1seq,2));
                    Mov.aom2offx = aom2offx;
            ...
            if GetResponse == 0
                if good_check == 1
                    ...
                    if(trial > ntrials)
                        ...
                        save(CFGname, 'CFG');
                        save([cd '\lastMappingname.mat'], 'matfname', 'psyfname','CFGname');
                        [plots] = plot_data(matfname, method);
...
function [dplots] = plot_data(matfname, method)
load(matfname);
plot_y = 325;
data_num = 1;
figure;
...
close(h)
trialIntMatrix = zeros(size(response_matrix,1),1,numcones);
questIntMatrix = zeros(size(response_matrix,1),1,numcones);
respMatrix = zeros(size(response_matrix,1),1,numcones);
questMeanMatrix = zeros(size(response_matrix,1),1,numcones);
questSDMatrix = zeros(size(response_matrix,1),1,numcones);
flagMatrix = zeros(size(response_matrix,1),1,numcones);

for col = 1:numcones;
    for row = 1:size(response_matrix,1);
        %search for empty cells
        if isempty(response_matrix(row,col).trialIntensity) == 1;
            response_matrix(row,col).trialIntensity = NaN;
        else
        end
        
        if isempty(response_matrix(row,col).questIntensity) == 1;
            response_matrix(row,col).questIntensity = NaN;
        else
        end
        
        if isempty(response_matrix(row,col).response) == 1;
            response_matrix(row,col).response = NaN;
        else
        end
        
        if isempty(response_matrix(row,col).questMean) == 1;
            response_matrix(row,col).questMean = NaN;
        else
        end
        
        if isempty(response_matrix(row,col).questSD) == 1;
            response_matrix(row,col).questSD = NaN;
        else
        end
        
        if isempty(response_matrix(row,col).trial_flag) == 1;
            response_matrix(row,col).trial_flag = NaN;
        else
        end
    end
end
...
for col = 1:numcones;
    for n = 1:size(response_matrix,1)
        trialIntMatrix(n,1,col) = response_matrix(n,col).trialIntensity;
        questIntMatrix(n,1,col) = response_matrix(n,col).questIntensity;
        respMatrix(n,1,col) = response_matrix(n,col).response;
        questMeanMatrix(n,1,col) = response_matrix(n,col).questMean;
        questSDMatrix(n,1,col) = response_matrix(n,col).questSD;
        flagMatrix(n,1,col) = response_matrix(n,col).trial_flag;
        video_name = {response_matrix(n,col).video_name};
...
if method == 'q';
    ...
    hold on
    for ii=1:numcones
        plot(trialIntMatrix2(:,1,ii), 'Color', color_script(round(ii/(size(response_matrix,2))*size(color_script,1)),:), 'LineWidth', linewidth);
    end
    ...
    set(leg,'Location', 'NorthEastOutside', 'FontSize', 8);
    ...
    % hold on
    % for ii=1:numcones
    %     plot(questIntMatrix2(:,1,ii), 'Color', color_script(round(ii/(size(response_matrix,2))*size(color_script,1)),:), 'LineWidth', linewidth);
    % end
    ...
    hold on
    for ii=1:numcones
        plot(questMeanMatrix2(:,1,ii), 'Color', color_script(round(ii/(size(response_matrix,2))*size(color_script,1)),:), 'LineWidth', linewidth);
    end
...
elseif method == 'm'
    %     mocs_mat = [trialIntMatrix respMatrix];
    %     mocs_mat = sortrows(mocs_mat,1);
    %     levels = unique(mocs_mat(:,1));
    %     pf_mat = zeros(size(levels,1),3);
    %     for n = 1:size(levels,1)
    %         [r] = find(A==levels(n));
    %         numpresent = size(r,2);
    %         numseen = sum(B(r));
    %         pf_mat(n,1) = levels(n);
    %         pf_mat(n,2) = numseen;
    %         pf_mat(n,3) = numpresent;
    %     end
    %
    % else
    %plot nothing
    if exist(matfname, 'file') == 2;        
        fitting_pal(matfname);
    else
        %do nothing
    end
end
...
clearvars -except data_fnames data_pnames data_num color_script plot_y
dplots = 1;