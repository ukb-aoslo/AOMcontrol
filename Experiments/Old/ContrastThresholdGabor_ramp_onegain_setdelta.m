function ContrastThresholdGabor_ramp_onegain_setdelta
...
staircasefactor = 1.2; %1; %1.5;
cuelength = 15; %was 30
% horizorvert = 1;

gainseqint = ones(CFG.npresent,1).*gain;
% gainseqint = [ones(CFG.npresent,1).*0.5; ones(CFG.npresent,1).*0.05;
% ones(CFG.npresent,1); ones(CFG.npresent,1).*1.5];
% gainseqint = [ones(CFG.npresent,1).*1; ones(CFG.npresent,1).*1; ones(CFG.npresent,1); ones(CFG.npresent,1).*1];
% gainseqint = [ones(CFG.npresent,1).*0.05; ones(CFG.npresent,1).*0.05; zeros(CFG.npresent,0.05); ones(CFG.npresent,1).*0.05];
...
%             cuelength = 10;
            cuehalfsize = 5;
                ...
                createStimulus(viddurtotal,horizorvert,fieldsize,mincontrast,maxcontrast,cuelength,cuehalfsize);
                if SYSPARAMS.realsystem == 1
...
function createStimulus(viddurtotal,horizorvert,fieldsize,mincontrast,maxcontrast,cuelength,cuehalfsize)
...
imSize = 64; %32;%32; %64; %150;                           % image size: n X n
fieldsizeDeg = imSize*fieldsize/512;
cpd=10;%12; %15;%20;                                  % target spatial frequency      
...
%     g(:,1:3)=0; g(:,(imSize-2):imSize)=0; g(1:3,:)=0; g((imSize-2):imSize,:)=0;
    ...
    %cue
    if contrast < (minintensity + cuelength*stepsize)
        g = g.*0;
