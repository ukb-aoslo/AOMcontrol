...
%             Mov.aom1seq = aom1seq;
                ...
                createStimulus(viddurtotal,horizorvert,thresholdGuess);
...
%     lastcorrect = correct;    
    elseif(GetResponse == 1)
        if(resp == kb_YesConst) %Right Arrow (Clockwise Horizontal)
            if horizorvert==1 %horiz
%                 response = 1;
                message1 = [Mov.msg ' - Horizontal Response - Correct'];
...
%                 response = 0;
                message1 = [Mov.msg ' - Horizontal Response - Incorrect'];
                %mar = mapping(stimulus,3);
                correct = 0;
                correctcounter = 0;
                GetResponse = 0;
        ...
        elseif(resp == kb_NoConst) %Left Arrow (Vertical)
            if horizorvert==0 %vert
%                 response = 1;
                message1 = [Mov.msg ' - Vertical Response - Correct'];
                %mar = mapping(stimulus,3);
                correct = 1;
                correctcounter = correctcounter + correct;
                if (correctcounter == 2)
...
%                 response = 0;
                message1 = [Mov.msg ' - Vertical Response - Incorrect'];
                %mar = mapping(stimulus,3);
                correct = 0;
                correctcounter = 0;
                GetResponse = 0;
...
function createStimulus(viddurtotal,horizorvert,thresholdGuess)

% viddurtotal = 30*1;
minintensity = thresholdGuess; %500; %25;
maxintensity = 1; %100;
stepsize = (minintensity-maxintensity)/viddurtotal;
...
imSize = 50; %100;                           % image size: n X n
cpd=10; %5;                                  % target spatial frequency        
...
gf=fspecial('gaussian',lambda+1,3); %std was 25
gf=gf/max(gf(:));
gf2=fspecial('gaussian',imSize,sigma);
gf2=gf2/max(max(gf2));

%%%% Pattern for drifting grating
...
i=2; %2;
% figure
for phase=minintensity:-stepsize:maxintensity %0:0.05:10
    
phaseRad = 0;
grating = sin( XYf + phaseRad);         % make 2D sinewave
g=grating;
   
    gf2=gf2/(phase.*max(max(gf2)));
%     gf2=gf2/(phase);
    g=g.*gf2;
