function JustContrastThreshold_062012
...
experiment = 'Experiment Name: Just Contrast Threshold';
...
fprintf(psyfid,'%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n', psyfname, experiment, subject, pupil,field, presentdur, maxintensity, iti, videoprefix, videodur);

if CFG.method == 'q'
    num_trials = ['Number of Trials: ' num2str(CFG.npresent)];
    fprintf(psyfid,'%s\n',num_trials);
    questparams = ['Staircase: Max Contrast: ' num2str(CFG.thresholdGuess), '    Intervals: ' num2str(CFG.npresent), '     Step Size: ' num2str(staircasefactor)];
    fprintf(psyfid,'%s\n',questparams);
end
...
%                     staircaseviddurtotal = staircaseviddurtotal;
                    theThreshold(trial,1) = trialContrast; %#ok<AGROW>
