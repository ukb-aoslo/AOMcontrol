...
antialias = CFG.antialiased; % For smooth circles. Has no effect on squares here
...
% Generate and return stimulus bitmap
    function s = makeStimulus (sheight, swidth, dotsize, dottype, dotposx, dotposy, antialias)
        ...
        elseif strcmp(dottype,'Circle')
            radius = floor(dotsize/2);
            [columnsInImage rowsInImage] = meshgrid(1:swidth, 1:sheight);
            for n = 1:size(dotposx,1)
                c = (rowsInImage - dotposy(n)).^2 ...
                    + (columnsInImage - dotposx(n)).^2 <= radius.^2;
                s = s+c;
            end
            if antialias == 1
