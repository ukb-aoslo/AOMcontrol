...
StimParams.wavfile = wavread('breep.wav');
...
if SYSPARAMS.realsystem == 1
    ...
    switch button,
        ...
        case 'FPGA'
            ...
            pause(0.1);
            aligncommand = ['UpdateOffset#2#' num2str(StimParams.aomoffs(2, 1)) '#' num2str(StimParams.aomoffs(2, 2)) '#'];   %#ok<NASGU>
            netcomm('write',SYSPARAMS.netcommobj,int8(aligncommand));
            pause(0.1);
            aligncommand = ['UpdateOffset#3#' num2str(StimParams.aomoffs(3, 1)) '#' num2str(StimParams.aomoffs(3, 2)) '#'];   %#ok<NASGU>
            netcomm('write',SYSPARAMS.netcommobj,int8(aligncommand));
            pause(0.1);
            command = ['UpdatePower#0#' num2str(SYSPARAMS.aompowerLvl(1)) '#'];            
...
if SYSPARAMS.realsystem == 1
    aligncommand = ['UpdateOffset#' num2str(SYSPARAMS.aomoffsvsel) '#' num2str(StimParams.aomoffs(SYSPARAMS.aomoffsvsel, 1)) '#' num2str(StimParams.aomoffs(SYSPARAMS.aomoffsvsel, 2)) '#'];   %#ok<NASGU>
    if SYSPARAMS.board == 'm'
...
if SYSPARAMS.realsystem == 1
    aligncommand = ['UpdateOffset#' num2str(SYSPARAMS.aomoffshsel) '#' num2str(StimParams.aomoffs(SYSPARAMS.aomoffshsel, 1)) '#' num2str(StimParams.aomoffs(SYSPARAMS.aomoffshsel, 2)) '#'];   %#ok<NASGU>
    if SYSPARAMS.board == 'm'
        MATLABAomControl32(aligncommand);
    else
        netcomm('write',SYSPARAMS.netcommobj,int8(aligncommand));
